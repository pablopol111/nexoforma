...
export default async function NutritionistClientDetailPage({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  return (
    <DashboardShell role="nutritionist" activeHref="/nutritionist" pageTitle={profile?.full_name ?? "Cliente"} pageDescription="Seguimiento completo del cliente." profileName={session.profile.full_name} profileSubtext={session.profile.email} actions={<RequestMeasurementsButton clientId={clientId} />}>
      ...
      <MeasurementsRadar entries={measurementEntries ?? []} />
      <BodyPanel referenceWeight={clientProfile?.reference_weight_kg ?? null} currentWeight={latest?.weight_kg ?? null} predictedWeight={predicted} ready={ready} latestMeasurements={latestMeasurement} previousMeasurements={previousMeasurement} />
      <section className="panel columns two">
        <div className="stack"><div className="panelHead"><h2>Histórico peso</h2></div><div className="tableWrap"><table><thead><tr><th>Fecha</th><th>Peso</th><th>Pasos</th><th>Comentario</th></tr></thead><tbody>{(dailyEntries ?? []).slice(-10).reverse().map((item) => <tr key={item.id}><td>{formatDate(item.entry_date)}</td><td>{formatNumber(item.weight_kg, 2)} kg</td><td>{formatSteps(item.steps)}</td><td>{item.comment ?? '-'}</td></tr>)}</tbody></table></div></div>
        <div className="stack"><div className="panelHead"><h2>Histórico medidas</h2></div><div className="tableWrap"><table><thead><tr><th>Fecha</th><th>Cintura</th><th>Cadera</th><th>Pecho</th><th>Comentario</th></tr></thead><tbody>{(measurementEntries ?? []).slice(0, 10).map((item) => <tr key={item.id}><td>{formatDate(item.entry_date)}</td><td>{formatNumber(item.waist_cm, 2)}</td><td>{formatNumber(item.hip_cm, 2)}</td><td>{formatNumber(item.chest_cm, 2)}</td><td>{item.comment ?? '-'}</td></tr>)}</tbody></table></div></div>
      </section>
