...
type ClientRow = { user_id: string; nutritionist_user_id: string };
type ProfileRow = { id: string; username: string; full_name: string; email: string };
type ClientProfileRow = { client_user_id: string; first_name: string | null; last_name: string | null; age: number | null; sex: string | null; height_cm: number | null; reference_weight_kg: number | null; target_weight_kg: number | null };
...
export default async function NutritionistClientDetailPage({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  return (
    <DashboardShell role="nutritionist" activeHref="/nutritionist" pageTitle={profile?.full_name ?? 'Cliente'} pageDescription="Vista adaptada de seguimiento del cliente." profileName={session.profile.full_name} profileSubtext={session.profile.email} actions={<RequestMeasurementsButton clientId={clientId} />}>
      ...
      <BodyPanel referenceWeight={clientProfile?.reference_weight_kg ?? null} currentWeight={latest?.weight_kg ?? null} predictedWeight={predicted} ready={ready} />
